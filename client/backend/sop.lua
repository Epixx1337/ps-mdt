...
RegisterNUICallback('getSOPCategories', function(data, cb)
    local result = Bridge.callback(resourceName .. ':server:getSOPCategories')
    cb(result or {})
end)
...
RegisterNUICallback('createSOPCategory', function(data, cb)
    ...
    local result = Bridge.callback(resourceName .. ':server:createSOPCategory', data)
...
RegisterNUICallback('updateSOPCategory', function(data, cb)
    ...
    data.id = nil
    local result = Bridge.callback(resourceName .. ':server:updateSOPCategory', categoryId, data)
    cb(result or { success = false })
end)
...
RegisterNUICallback('deleteSOPCategory', function(data, cb)
    if not data or not data.id then cb({ success = false }) return end
    local result = Bridge.callback(resourceName .. ':server:deleteSOPCategory', data.id)
    cb(result or { success = false })
end)
...
RegisterNUICallback('createSOPSection', function(data, cb)
    if not data then cb({ success = false }) return end
    local result = Bridge.callback(resourceName .. ':server:createSOPSection', data)
    cb(result or { success = false })
end)
...
RegisterNUICallback('updateSOPSection', function(data, cb)
    ...
    data.id = nil
    local result = Bridge.callback(resourceName .. ':server:updateSOPSection', sectionId, data)
    cb(result or { success = false })
end)
...
RegisterNUICallback('deleteSOPSection', function(data, cb)
    if not data or not data.id then cb({ success = false }) return end
    local result = Bridge.callback(resourceName .. ':server:deleteSOPSection', data.id)
    cb(result or { success = false })
end)

-- Get SOP settings (intro + version)
RegisterNUICallback('getSOPSettings', function(data, cb)
    local result = Bridge.callback(resourceName .. ':server:getSOPSettings')
    cb(result or {})
end)
...
RegisterNUICallback('updateSOPMission', function(data, cb)
    if not data then cb({ success = false }) return end
    local result = Bridge.callback(resourceName .. ':server:updateSOPMission', data.mission_statement or '')
    cb(result or { success = false })
end)
...
RegisterNUICallback('updateSOPIntro', function(data, cb)
    if not data then cb({ success = false }) return end
    local result = Bridge.callback(resourceName .. ':server:updateSOPIntro', data.introduction or '')
    cb(result or { success = false })
end)

-- Publish SOP (bump version)
RegisterNUICallback('publishSOP', function(data, cb)
    local result = Bridge.callback(resourceName .. ':server:publishSOP')
    cb(result or { success = false })
end)

-- Check SOP agreement status
RegisterNUICallback('checkSOPAgreement', function(data, cb)
    local result = Bridge.callback(resourceName .. ':server:checkSOPAgreement')
    cb(result or { agreed = true })
end)

-- Acknowledge SOP
RegisterNUICallback('acknowledgesSOP', function(data, cb)
    local result = Bridge.callback(resourceName .. ':server:acknowledgesSOP')
    cb(result or { success = false })
end)
