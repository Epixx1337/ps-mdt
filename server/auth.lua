...
function CheckAuth(source, silent)
    Bridge.debug('Checking MDT Authorization')
    local jobType = Bridge.getJobType(source)
    ...
    if jobType ~= Config.PoliceJobType and jobType ~= Config.MedicalJobType and not dojCheck then
        Bridge.debug('Access Denied for ID: ' .. source .. ', Name: ' .. Bridge.getPlayerName(source) .. ', not an authorized job type')
        if not silent then
            Bridge.notify(source, 'Access Denied: Authorized Personnel Only', 'error')
        end
        return false
    else
        Bridge.debug('Access Granted for ID: ' .. source .. ', Name: ' .. Bridge.getPlayerName(source) .. ', job type: ' .. tostring(jobType))
        return true
    end
end
...
function CheckPermission(source, permName)
    ...
    if Bridge.isBoss and Bridge.isBoss(source) then return true end

    local jobData = Bridge.getJobData and Bridge.getJobData(source) or nil
    local isBoss = false
    local gradeValue = 0
    ...
    local jobName = Bridge.getJobName(source) or 'police'
...
local function upsertProfileSession(src, action)
    ...
    local fullName = Bridge.getPlayerName(src)
    local callsign = Bridge.getMetadata(src, 'callsign')
    local job = Bridge.getJobData and Bridge.getJobData(src) or nil
    local jobName = job and job.name or Bridge.getJobName(src)
    local jobGrade = job and job.grade and job.grade.name or Bridge.getJobGradeName(src)
    ...
    if not ok or not profileId then
        Bridge.warn('Failed to upsert profile session for ' .. tostring(citizenid) .. ': ' .. tostring(profileId))
        return
    end
    ...
    if action == 'login' then
        ...
        if not okTx then
            Bridge.warn('Failed to create login session (transaction): ' .. tostring(errTx))
        end
    elseif action == 'logout' then
        ...
        if not okTx then
            Bridge.warn('Failed to update logout session (transaction): ' .. tostring(errTx))
        end
    end
end
...
RegisterNetEvent('ps-mdt:server:trackLogin', function()
    ...
    if Bridge.auditLog then
        Bridge.auditLog(src, 'mdt_login', 'profile', Bridge.getIdentifier(src), {})
    end
    -- FiveManage duty log
    local officerName = Bridge.getPlayerName(src) or 'Unknown'
    local citizenid = Bridge.getIdentifier(src) or 'N/A'
    local jobName = Bridge.getJobName(src) or 'Unknown'
...
RegisterNetEvent('ps-mdt:server:trackLogout', function()
    ...
    if Bridge.auditLog then
        Bridge.auditLog(src, 'mdt_logout', 'profile', Bridge.getIdentifier(src), {})
    end
    -- FiveManage duty log
    local officerName = Bridge.getPlayerName(src) or 'Unknown'
    local citizenid = Bridge.getIdentifier(src) or 'N/A'
...
Bridge.registerCallback(tostring(GetCurrentResourceName())..':server:checkAuth', function(source)
...
Bridge.registerCallback(tostring(GetCurrentResourceName())..':server:getMyPermissions', function(source)
    local src = source
    if not CheckAuth(src, true) then return { permissions = {} } end

    local jobName = Bridge.getJobName(src) or 'police'
    local jobData = Bridge.getJobData and Bridge.getJobData(src) or nil
    local gradeValue = 0
    local isBoss = false
    ...
    if isBoss or (Bridge.isBoss and Bridge.isBoss(src)) then
